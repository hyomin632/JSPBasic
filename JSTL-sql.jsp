...
<html>
	...
	<body>
		<h1>JSTL-sql</h1>
		<h2>sql:setDataSource - 데이터베이스 연결 정보 설정</h2>
		...
		<c:choose>
			<c:when test="${ avg ge 90 }">
				<c:set var="grd" value="수"></c:set></c:when>
			<c:when test="${ avg ge 80 }">
				<c:set var="grd" value="우"></c:set></c:when>
			<c:when test="${ avg ge 70 }">
				<c:set var="grd" value="미"></c:set></c:when>
			<c:when test="${ avg ge 60 }">
				<c:set var="grd" value="양"></c:set></c:when>
			<c:otherwise>
		...
		<sql:update dataSource="${mariadb}" var="cnt">
			...
			<sql:param value="${kor}"></sql:param>
			<sql:param value="${eng}"></sql:param>
			<sql:param value="${mat}"></sql:param>
			<sql:param value="${tot}"></sql:param>
			<sql:param value="${avg}"></sql:param>
			<sql:param value="${grd}"></sql:param>
		</sql:update>
